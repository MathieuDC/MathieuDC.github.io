...
var expertiseContents = {
    'commercial': {
        content: "<strong class='not-bold'>Karen BENHAMOU-KOSKAS</strong> accompagne la vie de l'entreprise dès sa constitution, jusqu' à sa dissolution en passant par tous les litiges qu'une société commerciale peut rencontrer pendant son activité."+
                ...
                "Elle vous assistera dans les formalités et les étapes  de la création de votre société."+
                ...
                "Karen BENHAMOU-KOSKAS rédige les actes de cession de fonds de commerce et de droit au bail et vous assistera en cas de litige nés de ces acquisitions ou cessions.",
        index: 1
    },
    'family': {
    ...
    'circulation': {
        content: "En partenariat avec un médecin spécialisé en  matière d'évaluation des préjudices corporels et avec une équipe de praticiens <strong class='not-bold'>Karen BENHAMOU-KOSKAS</strong> vous accompagnera dans toutes les étapes consécutives à un accident de la circulation :"+
                "l'expertise médicale qui permettra de quantifier vos préjudices ."+
                "l'indemnisation des préjudices qui s'inscrit dans un cadre amiable ou judiciaire."+
    ...
    'mediation': {
        ...
        "La décision de Justice obtenue ne répond pas à toutes les attentes du justiciable."+
        ...
        "Dans un second temps les parties se rencontrent en présence de<strong class='not-bold'>Karen BENHAMOU-KOSKAS</strong> dans son rôle d'Avocat-Médiateur pour, à l'issue de plusieurs rendez vous si nécessaire, mettre en place un accord émanant des  protagonistes eux mêmes qui satisfera chacun d'eux, sous le contrôle de l'Avocat Médiateur."+
        ...
        "<strong class='not-bold'>Karen BENHAMOU-KOSKAS</strong> est précursseur: elle fait partie de la première liste de médiateurs nommés par la cour d'Appel d'Aix en Provence.",
        index: 5
    }
}
...
function getAnchorPoint(idExpertise, rows){
    ...
    if(rows == 2){
        if(expertiseContents[idExpertise].index >= 1 && expertiseContents[idExpertise].index <= 3){
            return 'mdc-expertise-circulation';
        }
        return 'mdc-expertise-button-mediation';
    }
    if(rows == 3){
...
function insertExpertiseContents(){
    Object.keys(expertiseContents).forEach(idExpertise =>{
        let htmlEl = createExpertiseHtml(idExpertise);
        let idAnchor = getAnchorPoint(idExpertise, 1);
        let button = document.getElementById(idAnchor);
        button.parentNode.insertBefore(htmlEl, null);
    })
}

insertExpertiseContents();
